...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut pruned_headers = headers
        .map(|(header, version)| (header, prune_header(&format!("openvr/{header}"), version)));

    // mark types as 2.5.1 (arbitrary)
    let clientcore3 = prune_header("openvr/ivrclientcore_003.h", "2_5_1");
    pruned_headers[0].1.push_str(&clientcore3);
    let clientcore2 = prune_header("openvr/ivrclientcore_002.h", "2_0_10");
...
fn prune_header(header: &str, version: &str) -> String {
    ...
    let reader = BufReader::new(File::open(header).unwrap_or_else(|e| panic!("{header}: {e}")));
